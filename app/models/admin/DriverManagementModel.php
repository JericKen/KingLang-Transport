...
class DriverManagementModel {
    ...
    public function getAllDrivers() {
        $stmt = $this->conn->prepare("SELECT * FROM drivers ORDER BY full_name ASC");
        $stmt->execute();
    ...
    public function getDriverById($driverId) {
        $stmt = $this->conn->prepare("SELECT * FROM drivers WHERE driver_id = :driver_id");
    ...
    public function deleteDriver($driverId) {
        $stmt = $this->conn->prepare("DELETE FROM drivers WHERE driver_id = :driver_id");
        $stmt->bindParam(':driver_id', $driverId, PDO::PARAM_INT);
        return $stmt->execute();
    }
    ...
    public function getDriverStatistics() {
        ...
        // Total drivers
        $stmt = $this->conn->prepare("SELECT COUNT(*) as total FROM drivers");
        $stmt->execute();
        $stats['total'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        
        // Active drivers
        $stmt = $this->conn->prepare("SELECT COUNT(*) as active FROM drivers WHERE status = 'Active'");
        $stmt->execute();
        $stats['active'] = $stmt->fetch(PDO::FETCH_ASSOC)['active'];
        
        // Inactive drivers
        $stmt = $this->conn->prepare("SELECT COUNT(*) as inactive FROM drivers WHERE status = 'Inactive'");
        $stmt->execute();
        $stats['inactive'] = $stmt->fetch(PDO::FETCH_ASSOC)['inactive'];
        
        // On leave drivers
        $stmt = $this->conn->prepare("SELECT COUNT(*) as on_leave FROM drivers WHERE status = 'On Leave'");
        $stmt->execute();
        $stats['on_leave'] = $stmt->fetch(PDO::FETCH_ASSOC)['on_leave'];
        
        // Available drivers
        $stmt = $this->conn->prepare("SELECT COUNT(*) as available FROM drivers WHERE availability = 'Available'");
        $stmt->execute();
        $stats['available'] = $stmt->fetch(PDO::FETCH_ASSOC)['available'];
        
        // Assigned drivers
        $stmt = $this->conn->prepare("SELECT COUNT(*) as assigned FROM drivers WHERE availability = 'Assigned'");
        $stmt->execute();
