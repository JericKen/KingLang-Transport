...
class BusManagementModel {
    ...
    public function getAllBuses() {
        $stmt = $this->conn->prepare("SELECT * FROM buses ORDER BY name ASC");
        $stmt->execute();
    ...
    public function getBusById($busId) {
        $stmt = $this->conn->prepare("SELECT * FROM buses WHERE bus_id = :bus_id");
    ...
    public function deleteBus($busId) {
        try {
            ...
            $stmt = $this->conn->prepare("DELETE FROM buses WHERE bus_id = :bus_id");
            $stmt->bindParam(":bus_id", $busId, PDO::PARAM_INT);
            $result = $stmt->execute();
    ...
    public function getBusUsageStats() {
        ...
        $stmt = $this->conn->prepare("
            ...
            WHERE bk.date_of_tour >= :current_month AND bk.date_of_tour < :next_month
            GROUP BY b.bus_id
            ORDER BY booking_count DESC
        ");
    ...
    public function getBusAvailability($startDate, $endDate) {
        ...
        $stmt = $this->conn->prepare("SELECT COUNT(*) FROM buses WHERE status = 'Active'");
        $stmt->execute();
